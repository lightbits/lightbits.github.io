...
If you use quaternions you can do a renormalization (just calculate the length and divide by it).

The analog for rotation matrices is called orthogonalization. You can find many ways to orthogonalize a matrix on google, depending on if you want higher accuracy or simpler code. Here's a simple one, used in Peter Corke's [Robotics, Vision and Control](todo: link to website) toolbox:
